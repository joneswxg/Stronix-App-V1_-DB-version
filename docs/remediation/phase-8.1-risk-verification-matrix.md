...
| P0 | Template Plans remain immutable; copying, creating, and editing User Plans preserves ordered content and enforces authenticated ownership. | `PlanRepositoryTests` covers seeded templates, copy fidelity, real ownership, pseudo/missing-owner rejection, cross-user mutation denial, ordered action/set persistence, validation, and service adapter authentication. `CopyTemplatePlanUseCaseTests`, `UpdatePlanUseCaseTests`, `CreatePlanViewModelTests`, `EditPlanViewModelTests`, and `PlanViewModelTests` cover use-case and presentation-state seams. | Execute **Browse And Use Template Plan**, **Create Training Plan**, and **Edit Training Plan**. Reopen both the edited User Plan and its source Template Plan after relaunch. | No broad UI automation is required in this phase; navigation, field editing, and visual confirmation remain manual. |
...
| P1 | Body measurements are authenticated-user scoped; create/update/delete preserve canonical data; all user-scoped screens refresh without cross-account leakage. | `BodyMeasurementRepositoryTests` covers owner scoping, ordering, CRUD, typed validation, canonical timestamps, legacy date parsing, and non-disclosing failures. `BodyMeasurementViewModelTests` covers shared state, selection, refresh, recovery, reset behavior, and rejection of a delayed outgoing-scope load after reset. `UserScopeTransitionTests` verifies body-measurement state is empty at the shared User A → logged-out → User B boundary before the new scope loads. | Execute **Add Body Measurement** and **Shared Body Measurement State and Profile**, including edit/delete, overview/list/detail/chart refresh, User A → User B → User A switching, and relaunch persistence. | Cross-screen layout, chart interaction, selection presentation, and delayed mutation hardening remain manual or are completed by the dedicated Phase 8.6 ticket; Phase 8.3 covers the shared transition and stale-load boundary. |
